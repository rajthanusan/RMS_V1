...
export default function Reservation({ email }) { // Accept email as a prop
  const [formData, setFormData] = useState({
    ...
    time: '08:00am',
    message: '',
    email, // Include email in the initial form data
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('/api/reg-book-table', formData); // Email is included in formData
